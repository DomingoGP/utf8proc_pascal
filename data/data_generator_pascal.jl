...
function print_c_data_tables(io, sequences, prop_page_indices, prop_pages, deduplicated_props,
    ...
    print(io, "const utf8proc_sequences:array[0..",length(sequences.storage)-1,"] of utf8proc_uint16_t = ")
    write_c_index_array(io, sequences.storage, 8)
    print(io, "const utf8proc_stage1table:array[0..")
	print(io, length(prop_page_indices)-1)
	print(io, "] of utf8proc_uint16_t = ")
    write_c_index_array(io, prop_page_indices, 8)
    print(io, "const utf8proc_stage2table:array[0..")
	print(io, length(prop_pages)-1)
	print(io, "] of utf8proc_uint16_t = ")
    write_c_index_array(io, prop_pages, 8)

    print(io,"const utf8proc_properties:array[0..",length(deduplicated_props),"] of utf8proc_property_t = (\n",
        "  (category:UTF8PROC_CATEGORY_CN; combining_class:0; bidi_class:UTF8PROC_BIDI_CLASS_NULL; decomp_type:0; decomp_seqindex:UINT16_MAX; casefold_seqindex:UINT16_MAX; uppercase_seqindex:UINT16_MAX; lowercase_seqindex:UINT16_MAX; titlecase_seqindex:UINT16_MAX; comb_index:\$3FF;comb_length:0;comb_issecond:false; bidi_mirrored:false; comp_exclusion:false; ignorable:false; control_boundary:false; charwidth:1; ambiguous_width:false; pad:0; boundclass:UTF8PROC_BOUNDCLASS_OTHER; indic_conjunct_break:UTF8PROC_INDIC_CONJUNCT_BREAK_NONE;)\n")
    for prop in deduplicated_props
        print(io, " ,(",
              "category:",c_enum_name("CATEGORY", prop.category), "; ",
              "combining_class:",prop.combining_class, "; ",
              "bidi_class:",c_enum_name("BIDI_CLASS", prop.bidi_class), "; ",
              "decomp_type:",c_enum_name("DECOMP_TYPE", prop.decomp_type), "; ",
              "decomp_seqindex:",c_uint16(prop.decomp_seqindex), "; ",
              "casefold_seqindex:",c_uint16(prop.casefold_seqindex), "; ",
              "uppercase_seqindex:",c_uint16(prop.uppercase_seqindex), "; ",
              "lowercase_seqindex:",c_uint16(prop.lowercase_seqindex), "; ",
              "titlecase_seqindex:",c_uint16(prop.titlecase_seqindex), "; ",
              "comb_index:",c_uint16(prop.comb_index), "; ",
              "comb_length:",c_uint16(prop.comb_length), "; ",
              "comb_issecond:",prop.comb_issecond, "; ",
              "bidi_mirrored:",prop.bidi_mirrored, "; ",
              "comp_exclusion:",prop.comp_exclusion, "; ",
              "ignorable:",prop.ignorable, "; ",
              "control_boundary:",prop.control_boundary, "; ",
              "charwidth:",prop.charwidth, "; ",
              "ambiguous_width:",prop.ambiguous_width, "; ",
              "pad:","0; ", # bitfield padding
              "boundclass:",c_enum_name("BOUNDCLASS", prop.boundclass), "; ",
              "indic_conjunct_break:",c_enum_name("INDIC_CONJUNCT_BREAK", prop.indic_conjunct_break),
              ";)\n"
        )
    end
    print(io, ");\n\n")
	
    # calc number of items.  I don't know Julia so brute force
    num = -1;
    for dm0 in collect(keys(comb_mapping))
	     num += length(collect(keys(comb_mapping[dm0])))
#        for dm1 in collect(keys(comb_mapping[dm0]))
#            num += 1
#        end
    end
	
    print(io, "const utf8proc_combinations_second:array[0..",num,"] of utf8proc_int32_t = (\n")
    for dm0 in sort!(collect(keys(comb_mapping)))
        print(io, " ");
    ...
    print(io, "const utf8proc_combinations_combined:array[0..",num,"] of utf8proc_int32_t = (\n")
    for dm0 in sort!(collect(keys(comb_mapping)))
        print(io, " ");
        for dm1 in sort!(collect(keys(comb_mapping[dm0])))
            code = comb_mapping[dm0][dm1]
